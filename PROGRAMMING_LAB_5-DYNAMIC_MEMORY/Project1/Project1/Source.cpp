...
public:
	list_iterator find(const _Tp match_val, bool comp(const _Tp &item, const _Tp &query)) {
...
bool compare(const int &item, const int &match) {
	std::cout << item << ' ' << match << '\n';
	return item == match;
}
...
int main() {
	...
	for (auto i : int_db.data()) std::cout << i << ' ';
	std::cout << '\n';

	std::vector<int> lessThan5 = int_db.select([](int &filter) { return filter < 5; });
	std::cout << "Numbers in list less than 5: ";
	for (auto i : lessThan5) std::cout << i << ' ';
	std::cout << '\n';
#endif
}
